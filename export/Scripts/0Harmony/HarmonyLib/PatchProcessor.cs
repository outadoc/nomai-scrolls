...
namespace HarmonyLib
{
	public class PatchProcessor
	{
		...
		public static ILGenerator CreateILGenerator(MethodBase original)
		{
			...
			return new DynamicMethodDefinition("ILGenerator_" + original.Name, returnType, list.ToArray()).GetILGenerator();
		}
		...
		{
			DynamicMethodDefinition dynamicMethodDefinition = method.GetMethodPatcher().CopyOriginal();
			if (dynamicMethodDefinition == null)
			{
				return null;
			}
			return new ILManipulator(dynamicMethodDefinition.Definition.Body, debug: false).GetRawInstructions();
		}
